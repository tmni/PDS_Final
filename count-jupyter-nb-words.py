        ...
        if file.endswith(".ipynb") and not file.endswith("checkpoint.ipynb") :
            ...
            for cell in nb.worksheets[0].cells:
                if cell.cell_type == "markdown":
                    ...
                    code_lines_count += (len(cell['input'].replace('#', '').lstrip().split('\n')))
                    # code_lines_count += (len(list(filter(lambda x: '#' != x[0],cell['input'].replace('#', '').lstrip().split('\n')))))
...
print("{} Lines in notebooks' code" .format(total_code_lines))
